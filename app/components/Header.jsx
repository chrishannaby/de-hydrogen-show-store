  ...
  return (
    <nav className={className} role="navigation">
      ...
      {(menu || FALLBACK_HEADER_MENU).items.map((item) => {
        ...
        return (
          <NavLink
            className="header-menu-item font-normal "
            end
            key={item.id}
            onClick={closeAside}
            prefetch="intent"
...
function HeaderMenuMobileToggle() {
  return (
    <a
      className="header-menu-mobile-toggle flex-1 hover:no-underline"
      href="#mobile-menu-aside"
    >
      <Bars3Icon className="w-6 h-6 hover:scale-110 transition-transform" />
    </a>
  );
}
...
function CartBadge({count}) {
  return (
    ...
    >
      ...
      <span className="absolute top-[-0.3rem] right-[-0.3rem]  w-4 h-4   bg-white border border-black text-xs rounded-lg flex items-center justify-center">
        {count}
      </span>
    </a>
  );
